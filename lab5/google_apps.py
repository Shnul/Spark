...
spark = SparkSession.builder.master("local[*]").appName('google_apps').getOrCreate()
...
google_apps_df = spark.read.csv('s3a://lab5/googleplaystore/', header=True)
...
selected_df.write.parquet('s3a://lab5/task1/', mode='overwrite')
selected_df.show()
